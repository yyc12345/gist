...
* 原型中为了书写方便，使用四边形面，之后再在代码中转换成三角形面输出（第1，2，3个坐标桉树徐组成第一个三角形，第3，4，1个坐标按顺序组成第二个三角形）。四边形面顶点序使用右手定则确定正面指向（OpenGL默认用右手坐标系，默认用右手定则确认正面；DirectX默认用左手坐标系，默认用左手定则确认正面）
...
X和Y方向，大方块每次延伸5，小方块每次延伸2.5。Z方向则无论大小方块，都为延伸倍数x5。延伸方向的正负实际上是决定是否为延伸方向数值取相反数再加到原数值上。实际计算的时候遵循`x += unit * length * (direction ? 1 : -1)`
...
X和Y方向，大方块每次延伸1，小方块每次延伸0.5。Z方向则无论大小方块，都为延伸倍数x1。延伸方向的正负实际上是决定是否为延伸方向数值取相反数再加到原数值上。实际计算的时候遵循`u += unit * length * (direction ? 1 : -1)`
...
计算方法遵循`x += unit * (length - syncReduce) * (direction ? 1 : -1)`

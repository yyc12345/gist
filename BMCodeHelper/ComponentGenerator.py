...
for file in os.listdir("G:\\obj_export_text"):
    ...
    while True:
        ...
        if cacheSp[0] == 'v':
            v.append((float(cacheSp[1]), float(cacheSp[2]), float(cacheSp[3])))
        elif cacheSp[0] == 'vt':
            vt.append((float(cacheSp[1]), float(cacheSp[2])))
        elif cacheSp[0] == 'vn':
            vn.append((float(cacheSp[1]), float(cacheSp[2]), float(cacheSp[3])))
    ...
    while True:
        ...
        if cacheSp[0] == 'f':
            ...
            f.append((int(cachev3[0]) - 1, int(cachev3[1]) - 1, int(cachev3[2]) - 1,
                int(cachev2[0]) - 1, int(cachev2[1]) - 1, int(cachev2[2]) - 1,
                int(cachev1[0]) - 1, int(cachev1[1]) - 1, int(cachev1[2]) - 1))
